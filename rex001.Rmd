...
start with R-Studio's Console, where you may interact directly with R. We will then
move to text editor and start working on the simulation of a randomized experiment 
that we will continue to work on in the next several seminars. 
...
### The Console
You may interact directly with R through R-Studio's the Console (typically the 
...
#### Store data
a. Store a set of numbers in a vector called x
```{r}
...
b. Randomly select (with replacement) 10 integers 
...
c. Repeat 'red', 'green' four times and store them in a vector called condition
```{r}
...
Experienced R-users often use the `str()` function to remind them about what
...
abs(sqrt(10) / (pi - 4))
```
2. Use R's help function to find out what this function does. If the help 
...
Click
<a href="script1_0.R" target="_blank">here to view</a>
a script file (Script1), to be completed it in the following seminars (copy and
paste its content into a new script file).  
...
on residential area. The children are either from a high-income residential area
or a low-income area. 
