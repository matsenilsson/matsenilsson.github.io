...
Here's a user-made function that calculates sample statistics (three digits as
...
Here's another user-made function:
```{r}
...
Click
<a href="script1_3.R" target="_blank">here to view</a>
Script1 as we left it after the last seminar (if you missed it, please copy and
paste the content into a new script file). 

Today we will analyse true and estimated casual effects

1. Single-unit causal effects
2. Average treatment effect (ATE) and Average treatment effect on the treated
(ATT) in our sample and asymptotically (by increasing the sample size to a very 
large number)
3. Conduct as-treated and intent-to-treat analyses to estimate ATE and ATT 


### Exercises
Next time we will complicate things by introducing non-compliance. Specifically,
we will simulate a causal relationship between socioeconomic status (SES) and 
risk of being a never-taker (an individual who will not receive treatment, no matter
assignment; everyone else in our scenario will be a complier: treated as
assigned). Give it a try already now. Start by formulating a plausible model, 
then try to implement it in R-code (this is an implausible model: "Everyone low 
in SES is a never-taker, everyone high in SES is a complier").
